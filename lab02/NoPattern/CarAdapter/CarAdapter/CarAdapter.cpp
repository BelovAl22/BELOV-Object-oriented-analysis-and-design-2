...
class JapaneseCar : public Car {
    wstring name;
    double speed, engine, fuel, clearance, accel, price, weight;
public:
    JapaneseCar(wstring n, double s, double e, double f, double c, double a, double p, double w)
        : name(n), speed(s), engine(e), fuel(f), clearance(c), accel(a), price(p), weight(w) {
    }

    wstring getName() override { return name; }
    double getMaxSpeedKmh() override { return speed; }       
    double getEngineVolume() override { return engine; }         
    double getFuelConsumption() override { return fuel; }        
    double getClearance() override { return clearance; }         
    double getAcceleration() override { return accel; }          
    double getPrice() override { return price; }               
    double getWeight() override { return weight; }               
};

// ---------- American ----------
class AmericanCar : public Car {
    wstring name;
    double speedMph, engineCI, mpg, clearanceIn, accel60, price, weightLbs;
public:
    AmericanCar(wstring n, double s, double e, double m, double c, double a, double p, double w)
        : name(n), speedMph(s), engineCI(e), mpg(m), clearanceIn(c), accel60(a), price(p), weightLbs(w) {
    }

    wstring getName() override { return name; }

    // ← КОНВЕРТАЦИЯ ВНУТРИ КЛАССА, а не в адаптере
    double getMaxSpeedKmh() override { return speedMph * 1.60934; }
    double getEngineVolume() override { return engineCI * 0.016387; }
    double getFuelConsumption() override { return 235.2 / mpg; }
    double getClearance() override { return clearanceIn * 25.4; }
    double getAcceleration() override { return accel60 * 1.03; }
    double getPrice() override { return price; }
    double getWeight() override { return weightLbs * 0.453592; }
};

wstring compareCars(Car* c1, Car* c2)
{
    int s1 = 0, s2 = 0;
...
Car* createCar(int id)
{
    ...
    {
    case ID_TOYOTA:
        return new JapaneseCar(L"Toyota", 210, 3.3, 9, 230, 6.8, 12704520, 2500);
    case ID_HONDA:
        return new JapaneseCar(L"Honda", 226, 1.5, 6, 150, 8.4, 3970162, 1200);
    case ID_NISSAN:
        return new JapaneseCar(L"Nissan", 250, 3.0, 11, 126, 4.5, 67000000, 1634);

    case ID_FORD:
        return new AmericanCar(L"Ford", 202, 307, 15, 6, 4, 65000, 3900);
    case ID_DODGE:
        return new AmericanCar(L"Dodge", 118, 345, 15, 12, 6, 60000, 5500);
    case ID_CHEVY:
        return new AmericanCar(L"Chevrolet", 198, 376, 16, 5.3, 4, 70000, 3800);
    }
    return nullptr;
}
    ...
    case WM_COMMAND:
    {
            ...
            id == ID_FORD || id == ID_DODGE || id == ID_CHEVY)
        {
            ...
            if (selectFirst) {
                ...
                SetWindowTextW(hText, msg.c_str());
                selectFirst = false;
            }
            else {
                selectedCar2 = id;
        ...
        if (id == 10) {
            ...
            Car* c1 = createCar(selectedCar1);
            Car* c2 = createCar(selectedCar2);
            if (c1 && c2) {
                wstring result = compareCars(c1, c2);
                SetWindowTextW(hText, result.c_str());
                delete c1;  
                delete c2;
            }
            break;
